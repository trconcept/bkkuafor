...
let pool = null;
let store = { appointments: [], rateEvents: [] };
let writeQueue = Promise.resolve();
const loginAttempts = new Map();
const jsonError = (res, status, message, retryAfter) => {
    if (retryAfter)
        res.setHeader('Retry-After', String(retryAfter));
    return res.status(status).json({ ok: false, message });
};
const normalise = (value) => value.normalize('NFKC').trim().toLocaleLowerCase('tr-TR').replace(/\s+/g, ' ');
const parseCookies = (header) => (header || '').split(';').reduce((result, item) => {
...
const getDeviceId = (req, res) => {
    ...
    if (deviceId !== existing) {
        ...
        res.setHeader('Set-Cookie', `kuafor_device=${encodeURIComponent(deviceId)}; Max-Age=31536000; Path=/; HttpOnly; SameSite=Lax${secure}`);
    }
    return deviceId;
};
...
const text = (value, maxLength) => typeof value === 'string' && value.trim().length <= maxLength ? value.trim() : '';
...
const normalizePhone = (value) => value.replace(/\D/g, '').slice(0, 24);
const isObviousFakeMobile = (nationalNumber) => /^(\d)\1{9}$/u.test(nationalNumber) ||
...
const normalizeTurkishMobilePhone = (value) => {
    ...
    if (!input || !/^[+]?(?:\d|\()[\d\s().-]*\d$/u.test(input) || input.indexOf('+') > 0)
        return '';
    ...
    if (nationalNumber.startsWith('00')) {
        if (!nationalNumber.startsWith('0090'))
            return '';
    ...
    if (!/^5\d{9}$/u.test(nationalNumber) || isObviousFakeMobile(nationalNumber))
        return '';
    return `90${nationalNumber}`;
};
const normalizeBlockedPhone = (value) => {
...
const serialiseJson = (value) => JSON.stringify(value);
...
const saveStore = async () => {
    ...
    try {
        ...
        for (const event of store.rateEvents) {
            await connection.execute('INSERT INTO rate_events (timestamp_ms, ip, device_id) VALUES (?, ?, ?)', [event.timestamp, event.ip, event.deviceId]);
        }
...
const loadStore = async () => {
    const db = getPool();
    const [appointmentRows] = await db.query('SELECT * FROM appointments ORDER BY created_at DESC');
    const [rateRows] = await db.query('SELECT timestamp_ms, ip, device_id FROM rate_events');
    store = {
...
const cleanup = (now) => {
    ...
    store.appointments = store.appointments.filter((appointment) => appointment.status !== 'pending' ||
...
const hashPassword = (password, salt, iterations) => crypto.pbkdf2Sync(password, Buffer.from(salt, 'hex'), iterations, 64, 'sha512').toString('hex');
...
const setSessionCookie = (res, token) => {
    ...
    res.setHeader('Set-Cookie', `kuafor_admin_session=${encodeURIComponent(token)}; Max-Age=${SESSION_MAX_AGE_MS / 1000}; Path=/; HttpOnly; SameSite=Lax${secure}`);
};
...
const currentAdmin = async (req) => {
    ...
    if (!token)
        return null;
    const [rows] = await getPool().execute('SELECT username FROM admin_sessions WHERE token_hash = ? AND expires_at > UTC_TIMESTAMP() LIMIT 1', [hashSessionToken(token)]);
...
const requireAdmin = async (req, res, next) => {
    try {
        ...
        if (!username)
            return jsonError(res, 401, 'Yönetici oturumu geçersiz veya süresi dolmuş.');
...
app.post('/api/admin/login', async (req, res, next) => {
    try {
        ...
        const [rows] = await getPool().execute('SELECT username, password_hash, password_salt, password_iterations FROM admin_users WHERE username = ? LIMIT 1', [username]);
        const user = rows[0];
        ...
        await getPool().execute('INSERT INTO admin_sessions (token_hash, username, expires_at, created_at) VALUES (?, ?, DATE_ADD(UTC_TIMESTAMP(), INTERVAL 8 HOUR), UTC_TIMESTAMP())', [hashSessionToken(token), username]);
...
app.get('/api/admin/session', requireAdmin, (req, res) => res.json({ ok: true, username: res.locals.adminUsername }));
app.post('/api/admin/logout', async (req, res, next) => {
    try {
        const token = parseCookies(req.headers.cookie).kuafor_admin_session;
        if (token)
            await getPool().execute('DELETE FROM admin_sessions WHERE token_hash = ?', [hashSessionToken(token)]);
...
app.post('/api/admin/password', requireAdmin, async (req, res, next) => {
    try {
        ...
        const [rows] = await getPool().execute('SELECT password_hash, password_salt, password_iterations FROM admin_users WHERE username = ? LIMIT 1', [username]);
        const user = rows[0];
        ...
        await getPool().execute('UPDATE admin_users SET password_hash = ?, password_salt = ?, password_iterations = ?, updated_at = UTC_TIMESTAMP() WHERE username = ?', [password.hash, password.salt, password.iterations, username]);
        return res.json({ ok: true });
    }
    catch (error) {
        return next(error);
...
const getBlockedPhones = async () => {
    const [rows] = await getPool().query('SELECT phone FROM blocked_phones ORDER BY created_at DESC, id DESC');
...
app.post('/api/admin/blocked-phones', requireAdmin, async (req, res, next) => {
    try {
        ...
        if (phone.length < 10 || phone.length > 24)
            return jsonError(res, 400, 'Geçerli bir telefon numarası gereklidir.');
        await getPool().execute('INSERT IGNORE INTO blocked_phones (phone, created_by, created_at) VALUES (?, ?, UTC_TIMESTAMP())', [phone, String(res.locals.adminUsername)]);
    ...
    try {
        ...
        try {
            ...
            for (const phone of phones) {
                await connection.execute('INSERT INTO blocked_phones (phone, created_by, created_at) VALUES (?, ?, UTC_TIMESTAMP())', [phone, String(res.locals.adminUsername)]);
            }
            await connection.commit();
        }
        catch (error) {
...
app.post('/api/admin/media', requireAdmin, async (req, res, next) => {
    try {
        ...
        if (!match)
            return jsonError(res, 400, 'JPG, PNG, WEBP, GIF, MP4, WEBM veya OGG dosyası yükleyebilirsiniz.');
        ...
        await getPool().execute('INSERT INTO uploaded_media (id, mime_type, media_data, created_by, created_at) VALUES (?, ?, ?, ?, UTC_TIMESTAMP())', [id, match[1], data, String(res.locals.adminUsername)]);
...
app.get('/api/media/:id', async (req, res, next) => {
    try {
        if (!/^[a-f0-9]{32}$/u.test(req.params.id))
            return jsonError(res, 404, 'Görsel bulunamadı.');
        const [rows] = await getPool().execute('SELECT mime_type, media_data FROM uploaded_media WHERE id = ? LIMIT 1', [req.params.id]);
        if (!rows[0] || !Buffer.isBuffer(rows[0].media_data))
            return jsonError(res, 404, 'Görsel bulunamadı.');
...
app.get('/api/appointments/:trackingCode', async (req, res, next) => {
    try {
        ...
        const [rows] = await getPool().execute('SELECT * FROM appointments WHERE tracking_code = ? LIMIT 1', [code]);
        if (!rows[0])
            return jsonError(res, 404, 'Randevu bulunamadı.');
    ...
    try {
        const result = await serialise(async () => {
            ...
            if (ipRequests >= MAX_REQUESTS_PER_HOUR || deviceRequests >= MAX_REQUESTS_PER_HOUR) {
                await saveStore();
                return { error: [429, 'Çok kısa sürede fazla deneme yapıldı. Lütfen biraz sonra tekrar deneyin.', 60] };
            }
            store.rateEvents.push({ timestamp: now, ip, deviceId });
            if (!isRecord(req.body))
                return { error: [400, 'Geçersiz randevu verisi gönderildi.'] };
            const body = req.body;
            if (typeof body.website === 'string' && body.website.trim())
                return { error: [400, 'Randevu formu doğrulanamadı.'] };
            if (typeof body.clientStartedAt !== 'number' ||
                !Number.isFinite(body.clientStartedAt) ||
                now - body.clientStartedAt < MIN_FORM_TIME_MS ||
                now - body.clientStartedAt > MAX_FORM_TIME_MS)
                return { error: [400, 'Formu göndermeden önce lütfen birkaç saniye bekleyin.'] };
            const customerName = text(body.customerName, 100);
            if (typeof body.customerPhone !== 'string' || !body.customerPhone.trim()) {
                return { error: [400, 'Telefon numarası zorunludur.'] };
            }
            const rawCustomerPhone = text(body.customerPhone, 32);
            const customerPhone = normalizeTurkishMobilePhone(rawCustomerPhone);
            if (!customerPhone) {
                return { error: [400, '05xx xxx xx xx biçiminde geçerli bir cep telefonu girin.'] };
            }
            ...
            if (customerName.length < 3 || !date || !timeSlot || !stylist ||
                ...
                return { error: [400, 'Lütfen ad, telefon, hizmet, uzman, tarih ve saat bilgilerini kontrol edin.'] };
            ...
            if (selectedDate <= today || selectedDate > today + 90 * 24 * 60 * 60 * 1000) {
                return { error: [400, 'Lütfen önümüzdeki 90 gün içinden geçerli bir tarih seçin.'] };
            }
            const safeServices = services.map((service) => {
                ...
                if (!id || !name || !Number.isFinite(duration) || duration < 5 || duration > 1_440 || !Number.isFinite(price) || price < 0 || price > 1_000_000)
                    return null;
            ...
            if (safeServices.some((service) => !service) || !stylistId || !stylistName)
                return { error: [400, 'Seçilen hizmet veya uzman bilgisi geçersiz.'] };
            const blockedRows = await getPool().execute('SELECT id FROM blocked_phones WHERE phone = ? LIMIT 1', [normalizeBlockedPhone(customerPhone)]);
            if (blockedRows[0].length > 0)
                return { error: [403, 'Bu telefon numarası engellendi. Lütfen farklı bir iletişim bilgisi kullanın.'] };
            ...
            if (dailyAppointments.filter((appointment) => appointment.ip === ip).length >= MAX_DAILY_PER_IP || dailyAppointments.filter((appointment) => appointment.deviceId === deviceId).length >= MAX_DAILY_PER_DEVICE) {
                return { error: [429, 'Günlük randevu limitine ulaştınız. Değişiklik için salonumuzla iletişime geçebilirsiniz.'] };
            }
            ...
            if (pendingAppointments.filter((appointment) => appointment.ip === ip).length >= MAX_PENDING_PER_IP || pendingAppointments.filter((appointment) => appointment.deviceId === deviceId).length >= MAX_PENDING_PER_DEVICE) {
                return { error: [429, 'Aktif bir randevu talebiniz zaten bekliyor. Yeni talep için mevcut talebin sonuçlanmasını bekleyin.'] };
            }
            ...
            if (store.appointments.some((appointment) => appointment.customerPhone && crypto.createHash('sha256').update(normalise(normalizeTurkishMobilePhone(appointment.customerPhone))).digest('hex') === phoneHash && ['pending', 'approved'].includes(appointment.status))) {
                return { error: [409, 'Aynı telefon numarasından birden fazla aktif randevu talebi oluşturulamaz.'] };
            }
            if (store.appointments.some((appointment) => appointment.fingerprint === fingerprint && appointment.status !== 'cancelled'))
                return { error: [409, 'Bu bilgilerle aynı tarih ve saat için zaten bir randevu talebiniz var.'] };
            if (store.appointments.some((appointment) => appointment.status !== 'cancelled' && appointment.date === date && appointment.timeSlot === timeSlot && isRecord(appointment.stylist) && text(appointment.stylist.id, 80) === stylistId && (appointment.status !== 'pending' || appointment.lockExpiresAt > now))) {
                return { error: [409, 'Seçtiğiniz saat az önce başkası tarafından alındı. Lütfen başka bir saat seçin.'] };
            }
            ...
            const appointment = {
                ...
                date, timeSlot,
                totalPrice: safeServices.reduce((total, service) => total + service.price, 0),
    ...
    try {
        ...
        const appointment = await serialise(async () => {
            ...
            const value = {
                ...
                status: ['pending', 'approved', 'completed', 'cancelled'].includes(String(body.status)) ? String(body.status) : 'approved',
...
app.patch('/api/admin/appointments/:id', requireAdmin, async (req, res, next) => {
    try {
        const updated = await serialise(async () => {
            ...
            if (!appointment || !isRecord(req.body))
                return null;
            const body = req.body;
            if (typeof body.status === 'string' && ['pending', 'approved', 'completed', 'cancelled'].includes(body.status))
                appointment.status = body.status;
            if (typeof body.customerName === 'string')
                appointment.customerName = text(body.customerName, 100);
            if (typeof body.customerPhone === 'string')
                appointment.customerPhone = normalizePhone(text(body.customerPhone, 32));
            if (typeof body.date === 'string' && validDate(body.date))
                appointment.date = body.date;
            if (typeof body.timeSlot === 'string' && /^(?:[0-1]\d|2[0-3]):[0-5]\d$/u.test(body.timeSlot))
                appointment.timeSlot = body.timeSlot;
            if (typeof body.notes === 'string')
                appointment.notes = text(body.notes, 500);
            if (Array.isArray(body.services) && body.services.length <= 8)
                appointment.services = body.services;
            if (isRecord(body.stylist))
                appointment.stylist = body.stylist;
            if (typeof body.totalPrice === 'number' && Number.isFinite(body.totalPrice) && body.totalPrice >= 0)
                appointment.totalPrice = body.totalPrice;
            if (typeof body.priceNote === 'string')
                appointment.priceNote = text(body.priceNote, 160);
            await saveStore();
            return appointment;
        });
...
app.use((error, _req, res, next) => {
    ...
    if (status === 400 || status === 413)
        return jsonError(res, status, status === 413 ? 'Gönderilen veri çok büyük.' : 'Geçersiz veri gönderildi.');
    return next(error);
});
...
const start = async () => {
    ...
    if (process.env.ADMIN_PASSWORD) {
        const [rows] = await getPool().execute('SELECT id FROM admin_users WHERE username = ? LIMIT 1', [adminUsername]);
        if (!rows[0]) {
            const password = createPasswordRecord(process.env.ADMIN_PASSWORD);
            await getPool().execute('INSERT INTO admin_users (username, password_hash, password_salt, password_iterations, created_at, updated_at) VALUES (?, ?, ?, ?, UTC_TIMESTAMP(), UTC_TIMESTAMP())', [adminUsername, password.hash, password.salt, password.iterations]);
